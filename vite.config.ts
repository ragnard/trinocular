...
// Stamped into both bundles as `__TRINOCULAR_BUILD__`, read through `$lib/build`.
...
const build = {
  version: process.env.TRINOCULAR_VERSION || "0.0.0",
  commit: process.env.TRINOCULAR_COMMIT || gitHead(),
  source: pkg.homepage
};
...
export default defineConfig({
  plugins: [sveltekit()],
  define: {
    __TRINOCULAR_BUILD__: JSON.stringify(build)
  },
  server: {
    fs: {
      allow: ["packages"]
