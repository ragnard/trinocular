...
export class Session {
  ...
  async open(sql: string): Promise<void> {
    await this.page.evaluateOnNewDocument(
      (id: string, content: string) => {
        ...
        localStorage.setItem(
          `trinette:workspace:default:file:${id}`,
          JSON.stringify({ id, name: "perf.sql", content, connectionId: "", viewFormats: {} })
        );
        localStorage.setItem(
          "trinette:workspace:default:ui",
    ...
    await this.page.waitForFunction(
      () => !!document.querySelector(".trinette-statement-toolbar .run"),
  ...
  async runStatement(index = 0, timeoutMs = 300_000): Promise<void> {
    ...
    const clicked = await this.page.evaluate((i: number) => {
      const button = document.querySelectorAll<HTMLElement>(".trinette-statement-toolbar .run")[i];
      button?.click();
      return !!button;
    }, index);
