...
- **Inspector** (`DataViewer.svelte`): reads whatever the table has selected, at full window height, as one *document per selected row* — field/value pairs with structs and arrays flattened to dotted paths, headed by the row number the table shows so a value traces back to its row. It is a pane of the window rather than a region of the results, which is what lets the whole selection be readable at once and would let it serve a document type other than a SQL file. Closed it costs nothing: the 30px strip under the table already spells out the anchor cell, which is the common "read this one truncated value" case. `⌘I` toggles it, registered both as a monaco action and on `window` for the same reason `⌘P` is.
